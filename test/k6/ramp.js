 ...
 *
 * Each /postgres request needs up to 4 pool slots concurrently:
 ...
 *
 * Stages hold long enough to collect a stable p95 reading at each VU step.
 * Uses JSON response (?mode=ramp) to remove template-rendering overhead.
 *
 * Run:
...
export const options = {
    ...
    scenarios: {
        ramp: {
            ...
            startVUs:  1,
            stages: [
                { duration: '20s', target: 2  },  // 2 VUs  →  8 slots: baseline, no queuing
                { duration: '30s', target: 2  },  // hold   — read stable p95 at 2 VUs
                { duration: '20s', target: 3  },  // 3 VUs  → 12 slots: pool saturation starts
                { duration: '30s', target: 3  },  // hold   — first inflection point
                { duration: '20s', target: 5  },  // 5 VUs  → 20 slots
                { duration: '30s', target: 5  },  // hold
                { duration: '20s', target: 8  },  // 8 VUs  → 32 slots
                { duration: '30s', target: 8  },  // hold
                { duration: '20s', target: 10 },  // 10 VUs → 40 slots: hard ceiling
                { duration: '30s', target: 10 },  // hold   — max throughput measurement
...
export default function () {
    const res = http.get(`${BASE_URL}/postgres?mode=ramp`);
    checkOk(res);
}
