...
/**
 * Benchmark handler using the native pgsql Async\Pool adapter.
 *
 * Route: GET /postgres/pgsql[?action=setup|teardown&mode=concurrent|baseline|stress]
 */
final class PgsqlHandler implements RequestHandlerInterface
{
    ...
    private function renderResponse(array $results, array $queryProfiles, float $totalMs, string $responseType): ResponseInterface
    {
        if ($responseType === 'html' && $this->template !== null) {
            $html = $this->template->render('postgres::pgsql', [
                'users'       => $results['users'],
